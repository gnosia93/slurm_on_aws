...
AWS Graviton processors are custom-built by AWS to deliver the best price performance for cloud workloads. Graviton-based instance is 20% more cheaper than X86-based and provide better performance in same generation instances. From AWS Graviton3 processor, it optimized for ML workloads and provide twice the `Single Instruction Multiple Data (SIMD)` bandwidth while also supporting `bfloat16`. 
